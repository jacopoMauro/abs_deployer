...
guard : var_or_field_ref '?'                           # ClaimGuard
		// original line | 'duration' '(' min=pure_exp ',' max=pure_exp ')' # DurationGuard
    | 'duration' '(' pure_exp ',' pure_exp ')' # DurationGuard
    | e=pure_exp                                       # ExpGuard
...
feature_decl_attribute :
    // TYPE_IDENTIFIER IDENTIFIER 'in' '{' is+=boundary_val (',' is+=boundary_val)* '}' ';'
        TYPE_IDENTIFIER IDENTIFIER 'in' '{' boundary_val (',' boundary_val)* '}' ';'
